...
func SetupRouter(UserRouter Route, RoleRouter Route) chi.Router {
	...
	hotelServiceURL := env.GetEnv("HOTEL_SERVICE_URL")
	bookingServiceURL := env.GetEnv("BOOKING_SERVICE_URL")
	reviewServiceURL := env.GetEnv("REVIEW_SERVICE_URL")
	paymentServiceURL := env.GetEnv("PAYMENT_SERVICE_URL")
