...
import type { Hotel } from "@/stores/types";
...
export default function EditHotelPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api.get(`/api/v1/hotels/${hotelId}`);
        const h: Hotel = res.data?.data || res.data;
        setForm({
          name: h.name || "",
          address: h.address || "",
          location: h.location || "",
          ...
          ratingCount: h.ratingCount?.toString() || "0",
        });
        if (h.images?.length) {
          setImages(h.images.map((img, i) => ({ url: img.url, altText: img.altText || "", displayOrder: i })));
        }
      } catch {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await api.put(`/api/v1/hotels/${hotelId}`, {
        name: form.name,
  ...
  if (loading) {
    return <div className="p-8 text-muted animate-pulse">Loading...</div>;
  }
  ...
  return (
    <div>
      ...
      <h1 className="text-3xl font-serif font-semibold text-navy mb-8">Edit Hotel</h1>
